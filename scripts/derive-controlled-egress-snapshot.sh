...
TARGET="${CAPE_CONTROLLED_SNAPSHOT:-hardened-baseline-controlled-egress-v1}"
...
echo "Controlled snapshot: $SOURCE -> $TARGET (DNS 10.66.0.1, gateway 10.66.0.254)"
...
if ! virsh snapshot-info "$VM" "$TARGET" >/dev/null 2>&1; then
  ...
  command='powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$a=Get-NetAdapter | Where-Object Status -eq Up | Select-Object -First 1; Set-DnsClientServerAddress -InterfaceIndex $a.ifIndex -ServerAddresses 10.66.0.1; Get-NetRoute -InterfaceIndex $a.ifIndex -DestinationPrefix 0.0.0.0/0 -ErrorAction SilentlyContinue | Remove-NetRoute -Confirm:$false; New-NetRoute -InterfaceIndex $a.ifIndex -DestinationPrefix 0.0.0.0/0 -NextHop 10.66.0.254 -RouteMetric 10 | Out-Null; Clear-DnsClientCache"'
...
PY
  resolve="$(curl -fsS --max-time 30 -X POST --data-urlencode \
    'command=powershell.exe -NoProfile -Command "(Resolve-DnsName validation.winstdt.test -Type A).IPAddress"' \
    "http://$GUEST_IP:8000/execute")"
  printf '%s' "$resolve" | grep -q '192.168.125.10' || { echo 'controlled DNS validation failed' >&2; exit 1; }
